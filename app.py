...
company_name = st.sidebar.text_input("Nombre de la Empresa", value="Empresa Ejemplo S.A.")
...
num_years = st.sidebar.slider("Años de Proyección", min_value=3, max_value=10, value=5)
...
for i in range(num_years):
    ...
    with col1:
        g = col1.number_input(f"Año {i+1} Crec.", value=5.0, step=0.5, key=f"g_{i}") / 100.0
        growth_rates.append(g)
    with col2:
        m = col2.number_input(f"Año {i+1} EBIT", value=15.0, step=0.5, key=f"m_{i}") / 100.0
        ebit_margins.append(m)

st.sidebar.markdown("---")
st.sidebar.subheader("⚙️ Supuestos Financieros & Tasa de Descuento")

tax_rate = st.sidebar.number_input("Tasa de Impuestos (%)", value=25.0, step=1.0) / 100.0
capex_percent = st.sidebar.number_input("CapEx / Ingresos (%)", value=4.0, step=0.5) / 100.0
nwc_percent = st.sidebar.number_input("Δ NWC / Ingresos (%)", value=2.0, step=0.5) / 100.0
da_percent = st.sidebar.number_input("D&A / Ingresos (%)", value=3.0, step=0.5) / 100.0
wacc = st.sidebar.number_input("WACC - Costo Promedio del Capital (%)", value=10.0, step=0.5) / 100.0
terminal_growth_rate = st.sidebar.number_input("Tasa de Crecimiento Perpetua g (%)", value=2.5, step=0.1) / 100.0
net_debt = st.sidebar.number_input("Deuda Neta ($)", value=200000.0, step=10000.0)
...
try:
    ...
    st.line_chart(df_projections.set_index("Año")[["Flujo Caja Libre (FCF) ($)", "PV FCF ($)"]])
    ...
    with col_btn:
        ...
        if st.button("💾 Guardar en Base de Datos", type="primary"):
            ...
            if success:
                st.success(f"✅ Escenario '{scenario_name}' guardado exitosamente en TiDB Cloud.")
            else:
